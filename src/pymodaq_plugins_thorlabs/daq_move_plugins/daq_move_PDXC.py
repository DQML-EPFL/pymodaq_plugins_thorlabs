...
from pymodaq_plugins_thorlabs.hardware.PDXC_COMMAND_LIB import pdxc
...
class DAQ_Move_PDXC(DAQ_Move_base):
    ...
    def get_actuator_value(self) -> DataActuator:
        ...
        """
        position = [0]
        result = self.controller.GetCurrentPosition(0, position)
        if result < 0:
    ...
    def ini_stage(self, controller=None):
        ...
        """
        print("*** connecting to pdxc device ***")
        pdxcobj = pdxc()
        ...
        if self.is_master:  # is needed when controller is master
            self.controller = pdxcobj #  arguments for instantiation!)
            initialized = True
        else:
        ...
        print("Connected to device SN " +str(sn))
